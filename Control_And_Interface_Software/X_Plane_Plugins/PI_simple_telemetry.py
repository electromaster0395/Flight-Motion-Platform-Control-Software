...
class PythonInterface:
    ...
    def init_drefs(self):
        ...
        self.XformDesc =  ('Surge (g)', 'Sway (g)', 'Heave (g)', 'Roll rate (rad/s)', 'Pitch rate (rad/s)', 
                           'Yaw rate (rad/s)', 'Roll angle (rad)', 'Pitch angle (rad)' )
    ...
    def InputOutputLoopCallback(self, elapsedMe, elapsedSim, counter, refcon):
        ...
        msg =   msg + '\n' 
        #print(self.controller_addr)
        try:
            self.udp.send(msg, (UDP_IP, TARGET_PORT))
        except:
            pass
    ...
    def read_telemetry(self):
        try:       
            ...
            telemetry.append(named_data.DR_phi) # bank angle
            telemetry.append(named_data.DR_theta * -1) # pitch angle
            telemetry.append(named_data.DR_Rrad * -1)  # yaw_rate
