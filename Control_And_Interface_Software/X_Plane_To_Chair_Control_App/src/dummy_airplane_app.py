...
#NOTE: Again, its worth noting that in this project acceleration and translation are used somewhat arbitrarily as the airplane's linear accel
...
class AirplaneControl(QWidget):
    ...
    #Method which updates each element of the self.pose list to each respective slider value. This also sets the label text to the appropiate val
    ...
    def send_udp_data(self):
        data = list(self.pose_values.values())  # Convert dict_values to a list
        data[-2:] = [math.radians(value) for value in data[-3:]]  # Convert last two items to radians
